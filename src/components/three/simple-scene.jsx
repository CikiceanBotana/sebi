...
const SimpleScene = () => {
  ...
  useEffect(() => {
    ...
    try {
      loader.load(
        ...
        (gltf) => {
          ...
          const baseScale = isMobileRef.current ? 8 : 6.5; // Changed from 10 to 8 for mobile
          const scale = baseScale / maxDim;
          model.scale.setScalar(scale);
          initialScaleRef.current = scale;

          const xOffset = isMobileRef.current ? -0.02 : -0.05;
          model.position.set(
            (-center.x * scale) + xOffset,
            -center.y * scale + 0.18, // Moved down by adjusting Y offset from 0.2 to 0.18
            -center.z * scale
          );
    ...
    const handleResize = () => {
      ...
      if (modelRef.current && initialScaleRef.current) {
        const baseScale = isMobileRef.current ? 8 : 6.5; // Changed from 15 to 8 for mobile
        const newScale = (baseScale / 6.5) * initialScaleRef.current;
