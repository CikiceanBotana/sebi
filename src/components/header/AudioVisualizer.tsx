...
const AudioVisualizer: React.FC<AudioVisualizerProps> = ({ onBassUpdate }) => {
  ...
  const [currentSong, setCurrentSong] = useState<Song>({ 
    title: "Casanova", 
    path: "/music/casanova.mp3" 
  });

  useEffect(() => {
    const initializeAudio = async () => {
      try {
        const audio = new Audio(currentSong.path);
        audio.loop = true;
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-[384px] h-12">
      <div className="flex flex-row items-center w-full">
        ...
        <div className="relative flex-grow">
          <canvas
            ...
            width="384"
            height="32"
            className="w-full h-8 rounded-md border border-[#047A6E]/20"
          />
          <button
            onClick={togglePlay}
